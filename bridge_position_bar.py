...
# データの定義
labels = ['1st', '2nd']
values = [0.480, 0.347]  # データをmmに更新

# グラフの基本設定（height_ratiosを1:4に変更）
fig, ax = plt.subplots(nrows=2, figsize=(4, 4), dpi=160, sharex='col',
...
ax[1].set_yticks([0.2])  # 目盛りを点のみに変更

# 上段のプロット領域
ax[0].set_ylim(0.34, 0.5)
ax[0].set_yticks([0.35, 0.4, 0.45])  # 目盛りを3点のみに変更
...
# ラベルの追加（修正部分）
fig.text(0.02, 0.5, 'position change (mm)', rotation=90, va='center')
ax[0].set_title('最大変位比較')

# 左側のマージンを調整して、y軸ラベルが見やすくなるようにする
